...
;(function (global) {
  ...
  function configApiBase() {
    ...
    let base = String(cfg.apiBase || '').trim().replace(/\/$/, '')
  ...
  function resolveProxyUrl() {
    return `${proxyBase()}/api/check-md5`
  }
  ...
  global.DynaServer = {
    ...
    setOnline(ok, hasJwt) {
      ...
      if (hosted) {
        if (!apiBase) {
          el.innerHTML =
            '<strong>Payment API not set up.</strong> Deploy the <code>api/</code> folder to <a href="https://vercel.com" target="_blank" rel="noopener">Vercel</a>, add <code>BAKONG_TOKEN</code> + <code>BAKONG_EMAIL</code>, then set <code>apiBase</code> in <code>standalone/bakong.config.js</code>. See README.'
        } else if (this.online) {
          el.innerHTML =
            '<strong>API online but no JWT.</strong> In Vercel → Project → Settings → Environment Variables, add <code>BAKONG_TOKEN</code> (from <code>node scripts/bakong-token.mjs your@email.com</code>) and <code>BAKONG_EMAIL</code>, then redeploy.'
        } else {
          el.innerHTML =
            '<strong>Cannot reach payment API.</strong> Check <code>apiBase</code> in <code>standalone/bakong.config.js</code> matches your Vercel URL. <button type="button" class="server-retry-btn" id="serverRetry">Retry</button>'
        }
        return
      }
